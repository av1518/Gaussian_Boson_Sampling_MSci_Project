...
for i in tqdm(loss):  
    ...
    div = kl_divergence(ground_distr, greedy_distr)
    distances.append(distance)
    divergences.append(div)

np.save(f'greedy_success_fixed_n_clicks_distance_n={n_modes}_cut={cutoff}_n_clicks={n_fixed}_L={L}_s={s}', distances)
np.save(f'greedy_success_fixed_n_clicks_kl_n={n_modes}_cut={cutoff}_n_clicks={n_fixed}_L={L}_s={s}', divergences)
...
plt.figure()
plt.plot(loss, divergences, 'o-', label = f'Modes = {n_modes}, Cutoff = {cutoff} ')
plt.xlabel('Loss')
plt.ylabel(r'$\mathcal{KL}$(Greedy,Ground)')
plt.legend()
plt.show()
