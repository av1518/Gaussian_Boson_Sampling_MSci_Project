...
kl = np.load('kl_greedy,ground_n=4_cut=6,repetitions=100,range =0.3,_gate_error,numberofpoints= 30.npy',allow_pickle=True)
def f(x,a,b):
    return a*x**2 + b 
...
er = np.array(generate_random_array(30, 0.01,0.02))
####
with plt.style.context(['science']):
    plt.figure(figsize=[8, 6])
    plt.xticks(size=16)
